...
class Mcu_model extends CI_Model {
	...
	private function _get_datatables_query_mcu()
	{
		if ( $this->session->userdata('role') == 'superuser' ) {
			...
			$this->db->select('mcus_v1.id as id, medical_record_number, id_number_patient, name_patient, mcus_v1.is_fit as mcu_is_fit, patients.id_number as patient_id_number, patients.name as patient_name, type_examination');
			$this->db->from('mcus_v1');
			$this->db->join('patients', 'patients.id=mcus_v1.id_patient', 'left');
